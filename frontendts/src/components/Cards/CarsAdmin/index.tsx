...
import { CarsAdminProps } from '../../Props';

function CardCars({ tittle, rentPerDay, id, updateAt, image }: CarsAdminProps) {
	const navigate = useNavigate();
	const token = localStorage.getItem('token');

	const handleOnClick: (id: number) => void = (id) => {
		navigate('/admin/cars/edit/' + id);
	};

	const handleOnDelete: (id: number) => void = (id) => {
	...
	return (
		<Card className={carsCard}>
			...
			<Card.Body className={carsBody}>
				...
				<Row>
					<Col>
						<Button variant={2} width="100%" onClick={() => handleOnDelete(id)}>
              Delete
						</Button>
					</Col>
					<Col>
